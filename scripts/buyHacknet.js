...
function help(ns) {
  ns.tprint(
    "This script helps you manage your Hacknet Nodes. " +
...
function response(ns, func, result) {
  switch (func) {
    case "buyNode":
      ns.tprint(
        result ? "✔️Purchased one node" : "You cannot afford a node.😢"
      );
      break;
    case "buyNodes":
      ns.tprint(
        result > 0
          ? `✔️Purchased ${result} nodes.`
          : "You cannot afford a node.😢"
      );
      break;
    case "upgradeNodes":
      ns.tprint(
        result
          ? "✔️Nodes Upgraded"
          : "⭐You are already upgraded as much as you can afford."
      );
      break;
    default:
      if (result === "Not Found") {
        ns.tprint(`${func} is not a valid option`);
      }
  }
}
